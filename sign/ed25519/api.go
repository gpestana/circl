...
func (e *Ed25519) KeyGen(public *PubKey, private *PrivKey) {
...
// keys of the signer.
func (e *Ed25519) Sign(msg []byte, public *PubKey, private *PrivKey) *Signature {
	k := sha512.Sum512(private[:])
	e.clamp(k[:])
	H := sha512.New()
	_, _ = H.Write(k[Size:])
...
// Verify returns true if the signature is valid. Another failure case is that
// the public key cannot be decoded.
func (e *Ed25519) Verify(msg []byte, public *PubKey, sig *Signature) bool {
	var P pointR1
	...
	hRAM := H.Sum(nil)
	e.reduceModOrder(hRAM[:])
	if ok := e.verifyRange(sig[Size:]); !ok {
		return false
	}
	var Q pointR1
	P.neg()
...
func (e *Ed25519) red512(x *[8]uint64, full bool) {
...
func (e *Ed25519) verifyRange(x []byte) bool {
	order := [Size]byte{
