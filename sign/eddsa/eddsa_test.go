...
func TestDevelEd(t *testing.T) {
	...
	sig := scheme.Sign(msg, &public, &private)
	t.Logf("sig: %x\n", sig)
	got := scheme.Verify(msg, &public, sig)
	t.Logf("got: %v\n", got)
}

type vector struct {
	name_test        string
	signature_scheme signScheme
	private_key      []byte
	public_key       []byte
	signature        []byte
	message          []byte
...
var vectorsed25519 = [...]vector{
	vector{
		name_test:        "-----TEST 1",
		signature_scheme: schemeEd25519,
		private_key: []byte{
...
func TestEd25519(t *testing.T) {
	...
	t.Run("keygen", func(t *testing.T) {
		...
		for _, v := range vectorsed25519 {
			copy(private[:], v.private_key)
			scheme.KeyGen(&public, &private)
			got := public
			copy(want[:], v.public_key)

			if got != want {
				test.ReportError(t, got, want, v.private_key)
			}
		}
	})
	...
	t.Run("sign", func(t *testing.T) {
		...
		for _, v := range vectorsed25519 {
			copy(private[:], v.private_key)
			copy(public[:], v.public_key)
			sig := scheme.Sign(v.message, &public, &private)
			got := *sig
			copy(want[:], v.signature)

			if got != want {
				test.ReportError(t, got, want, v.name_test)
			}
		}
	})
	...
	t.Run("Verify", func(t *testing.T) {
		...
		for _, v := range vectorsed25519 {
			...
			got := scheme.Verify(v.message, &public, &sig)
			want := true
...
func BenchmarkEd4448(b *testing.B) {
	var scheme Ed448
	var public Pk448
	var private Sk448
	var sig Sig448
